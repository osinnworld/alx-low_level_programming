 ...
 */
shash_table_t *shash_table_create(unsigned long int len)
{
	shash_table_t *sht;
	unsigned long int i;
	...
	sht->shead = NULL;
	sht->stail = NULL;
	sht->array = malloc(sizeof(shash_node_t) * len);
	if (sht->array == NULL)
	{
		free(sht);
		return (NULL);
...
void shash_table_delete(shash_table_t *ht)
{
	...
	{
		while (ht->array[len] != NULL)
		{
			next = ht->array[len]->next;
			free(ht->array[len]->key);
